...
//  function 
  ...
  for(const id of Object.keys(results)){
    if (id === '59e462c64a42a91ef5f4d8d0') console.log('printing array toQuery.',results[id].topic._id);
    const body = {
      query: {
        dis_max: {
          queries: [
            {
              terms: {
                topic_id: `${results[id].topic._id}`
              }
            },
            {
              terms: {
                sub_id: `${results[id].courseType._id}`
              }
            },
            {
              terms: {
                author_id: `${results[id].curator._id}`
              }
            }
          ]
        }
