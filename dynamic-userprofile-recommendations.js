...
async function createEmailData() {
  ...
  for (const id of Object.keys(results)) {
    ...
    tempGrade.push(results[id].grade);
    const body = {
      query: {
        dis_max: {
          queries: [
  ...
  console.log('<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< fetched courses data: ', JSON.stringify(fetchedCourses,null,2));
  ...
  for (const id in Object.keys(results)) {
    ...
    for (const course in user.recommendCourses) {
      ...
      emailData[`thumbnail${course}`] = user.recommendCourses[course].thumbnail.url || user.recommendCourses[course].thumbnail.thumb;
      ...
      console.log('course url........................',emailData[`courseUrl${course}`]);
...
async function getProminent() {
  ...
  userIds = await collectionUser.find({}, { _id: true, email: true, firstName: true , grade: true}).toArray();
